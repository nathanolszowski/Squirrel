...
def url_actualise_user_agents ():
    ...
    # Récupère la dernière liste à jour
...
def lister_user_agent (url_actuelle_agents):
    ...
    # Récupère la liste des user-agents
...
def verifier_cache_user_agents (derniere_url_actuelle):
    try:
        liste_user_agent = []
        ...
        if os.path.exists("user_agent.json"):
            with open("user_agent.json", "r", encoding="utf-8") as f:
                contenu = json.load(f)
                url_actualise_fichier = next(iter(contenu), None)

                # Si l'URL est la même que celle du sitemap actuel, on utilise le cache
                if url_actualise_fichier == derniere_url_actuelle:
                    liste_user_agent = contenu[url_actualise_fichier]
                else:
                    # Sinon, on met à jour
